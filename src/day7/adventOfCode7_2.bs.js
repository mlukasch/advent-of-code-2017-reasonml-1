...
function checkChildBalance(x) {
  return List.sort_uniq((function (param, param$1) {
                return Caml_obj.caml_int_compare(param[1], param$1[1]);
              }), List.map((function (x) {
                    var item = findByName(x);
                    var omg = List.fold_left((function (a, b) {
                            return a + ("," + Pervasives.string_of_int(b));
                          }), "", List.map((function (q) {
                                return getChildWeight(findByName(q));
                              }), item[2]));
                    return /* tuple */[
                            item[0],
                            getChildWeight(item),
...
function isBalanced(_item) {
  while(true) {
    ...
    var name = item[0];
    if (name !== "ahnofa") {
      var parent = List.find((function(name){
          return function (param) {
            return containsChild(name, param);
          }
          }(name)), nodes);
      var list = checkChildBalance(item[2]);
...
exports.checkChildBalance = checkChildBalance;
