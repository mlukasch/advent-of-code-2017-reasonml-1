...
function isNode(param) {
  return +(List.length(param[2]) > 0);
}

var leaves = List.filter(isLeaf)(input);

var nodes = List.filter(isNode)(input);
...
function findByName(name) {
  ...
  try {
    ...
    if (exn === Caml_builtin_exceptions.not_found) {
      tmp = Pervasives.exit(1);
    } else {
      throw exn;
    }
  }
  return Curry._1(tmp, input);
}

function childrenToItems(param) {
  return List.map(findByName, param[2]);
}

function containsChild(nameToFind, param) {
  return List.exists((function (x) {
                return Caml_obj.caml_equal(nameToFind, x);
              }), param[2]);
}
...
function findParentByName(name) {
  try {
    return List.find((function (param) {
                  return containsChild(name, param);
                }), nodes);
  }
  catch (exn){
    if (exn === Caml_builtin_exceptions.not_found) {
...
function getChildWeight(x) {
  var items = childrenToItems(x);
  return List.fold_left((function (a, b) {
                if (isLeaf(b)) {
                  return a + b[1] | 0;
                } else {
                  return a + getChildWeight(b) | 0;
                }
              }), x[1], items);
}

function checkIndexMatch(compareTo, parent) {
  return List.for_all((function (item) {
                var index = item[1];
                var name = item[0];
                var childWeight = getChildWeight(item);
                if (childWeight !== compareTo) {
                  console.log(List.fold_left((function (a, param) {
                              return a + ("," + (param[0] + (":" + (Pervasives.string_of_int(param[1]) + (":" + Pervasives.string_of_int(param[2]))))));
                            }), "", List.map((function (i) {
                                  return /* tuple */[
                                          i[0],
                                          i[1],
                                ...
                                }), childrenToItems(parent))));
                  if (childWeight > compareTo) {
                    var diff = compareTo - childWeight | 0;
                    var answer = index + diff | 0;
                    console.log("Mismatch: " + (String(name) + (" with " + (String(index) + (" should be ___ " + (String(answer) + " ___"))))));
                  } else {
                    var diff$1 = childWeight - compareTo | 0;
                    var answer$1 = index + diff$1 | 0;
                    console.log("Mismatch: " + (String(name) + (" with " + (String(index) + (" should be ___ " + (String(answer$1) + " ___"))))));
                  }
                  Pervasives.exit(1);
                }
                return +(compareTo === childWeight);
              }), childrenToItems(parent));
}

function isBalanced(_item) {
  while(true) {
    var item = _item;
    var parent = findParentByName(item[0]);
    var nodeWeight = getChildWeight(item);
    checkIndexMatch(nodeWeight, parent);
    _item = parent;
    continue ;
...
exports.findParentByName = findParentByName;
exports.getChildWeight   = getChildWeight;
exports.checkIndexMatch  = checkIndexMatch;
